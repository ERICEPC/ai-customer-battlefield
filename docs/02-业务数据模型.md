...
> 状态：逻辑模型与物理基线已确认；`0001`–`0002` 已实现，`0003`–`0006` 按计划继续
>
> 更新日期：2026-08-31
>
...
- 所有 13 张当前业务表使用复合租户外键，并由 PGlite 行为测试与真实 PostgreSQL CI 从空库重建；
...
- `0003`–`0006` 尚未实现，跟进确认、分析动作、通知审计、周报问数和导入不能误称已经落地。
...
联系人换组织与跨组织场景首期保留完整任职历史，但只实现当前/历史查询和有效期约束，不扩展成通用主数据管理。逻辑模型、状态机、租户边界、物理类型、索引和迁移顺序已经闭合；租户/身份与经营核心的 migration、repository、API 和目录页面已经实现。后续 `0003`–`0006` 继续补齐确认、分析动作、通知审计、周报问数和导入；尚未确认的周期、阈值和部署形态均被隔离为版本化配置或 adapter，不要求推翻核心表。
