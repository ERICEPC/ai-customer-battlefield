...
> 状态：逻辑模型与物理基线已确认；`0001`–`0005` 已实现，`0006` 及后续配置/导入增量按计划继续
>
> 更新日期：2026-08-31
>
...
| 逻辑表 | 主要字段 | 说明 |
|---|---|---|
| `action_proposals` | id、entity_id、opportunity_id、title、description、suggested_owner_id、suggested_priority、suggested_planned_at、source_battle_state_version_id、status、version_no、proposed_at、expires_at、decision_* | 分析生成且绑定不可变作战状态版本的建议动作，尚未进入待办 |
...
| `reminder_policy_versions` | id、policy_key、version_no、name、status、nodes、effective_at、published_by | 不可变已发布提醒策略；当前只发布到期节点，其他节点保留 schema 扩展位 |
| `reminder_instances` | action_id、policy_version_id、action_version_no、recipient_user_id、kind、remind_at、channels、status、attempt_count、claim_*、notification_event_id、dedupe_key | 每次具体提醒，支持租约恢复、取消、去重、重试与死信 |
...
| `notification_template_versions` | 按 template_key、channel 发布的不可变标题、正文、深链和优先级模板 |
...
- `0001_foundation.sql` 已实现租户、组织单元、用户、有效期成员关系、渠道地址、RLS helper 和强制租户策略；
- `0002_customer_operations.sql` 已实现经营对象类型/对象、责任关系、联系人/任职、商机/责任关系和阶段历史；
- `0003_followup_confirmation.sql` 已实现来源、草稿/修订、正式跟进、更正、参与人/商机关联、事实/证据、幂等、审计、领域事件和 Outbox 的物理基线；持久化 Store、REST API 与 Web 人工确认工作台已经贯通；
- `0004_battle_analysis_actions.sql` 已实现分析运行、信号、不可变作战状态与当前投影、证据关联、建议动作、正式动作和状态历史；对应 Store、REST API、地图与动作决策工作台已经贯通；
- `0005_reminders_notifications.sql` 已实现提醒策略版本、通知模板版本、提醒实例、通知事件和渠道投递；独立 Worker、站内通知 API/Web 与可选飞书 adapter 已经贯通；
- 当前共 40 张表；除租户根表外，所有带租户归属的关系都使用复合租户外键，并由 PGlite 行为测试与真实 PostgreSQL 18 CI 从空库重建；
- Kysely 只位于数据库 adapter，经营对象目录查询同时携带显式 `tenant_id` 并运行在 transaction-local RLS 上下文；
- migration runner 使用事务 advisory lock 与 SHA-256 历史校验，已发布 SQL 文件禁止修改；
- `sales_weekly_progress` 受控问数已经使用现有正式事实、责任关系、状态历史和审计表落地，不新增一次性查询结果表；周报版本、可配置 Prompt/模型/通用规则、历史导入和外部映射仍等待后续 migration，不能误称已经落地。当前提醒只发布 `due + 0 分钟` 的明确基线，提前、逾期和升级虽可由节点 schema 表达，但业务参数尚未确认。
...
| 建议动作 | `pending_confirmation → accepted / rejected / expired` | 只允许接受一次；接受与正式动作创建同事务 |
| 经营动作 | `planned ↔ in_progress → completed / cancelled` | 完成和取消为终态；改期、改负责人和状态变化均写历史 |
| 分析运行 | `running → completed / failed / superseded` | 旧输入版本即使成功也只能进入 `superseded`，不能覆盖当前投影 |
| 提醒实例 | `scheduled / failed → processing → notified / failed / cancelled / dead_lettered` | 去重键唯一；失败按策略重试；动作完成或取消会取消未通知实例；改期命令尚未实现 |
| 通知投递 | `pending / failed → processing → delivered / failed / dead_lettered` | 每渠道独立投递；外部渠道失败不回滚业务事务或站内投递 |
...
4. 追加审计、`followup.confirmed.v1` 领域事件与 Outbox；
5. 用户或后续受控调度通过分析接口读取最新确认事实，写入信号、不可变作战状态版本、当前投影和待确认建议；
6. 建议接受事务创建正式动作、状态历史、审计、领域事件和 Outbox；
7. 独立 Worker 消费动作 Outbox 生成提醒实例，并在到期后原子形成通知事件与站内投递；外部渠道在独立投递行上重试或死信。
...
4. `0004_battle_analysis_actions`：分析、信号、作战状态、建议动作、正式动作和状态历史；
5. `0005_reminders_notifications`：提醒/模板版本、提醒实例、站内通知和外部渠道投递；
6. `0006_reports_queries_imports`（计划）：周报、管理问数、可配置运行版本、导入批次和外部映射。

每个 migration 都要有前向执行测试、关键约束测试和从空库重建测试。已发布 migration 不修改内容；任何变化新增 migration，并在需要时提供数据回填、兼容期和回滚说明。

## 24. Phase 2 结论

联系人换组织与跨组织场景首期保留完整任职历史，但只实现当前/历史查询和有效期约束，不扩展成通用主数据管理。逻辑模型、状态机、租户边界、物理类型、索引和迁移顺序已经闭合；`0001`–`0005` 已贯通租户/身份、经营核心、跟进确认、作战分析、动作决策、到期提醒、站内通知和可选飞书投递。后续迁移补齐周报问数、配置发布和导入；尚未确认的周期、阈值和部署形态继续隔离为版本化配置或 adapter，不要求推翻核心表。
