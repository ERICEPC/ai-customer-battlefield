...
> 状态：逻辑模型与物理基线已确认；`0001`–`0003` 已实现，`0004`–`0006` 按计划继续
>
> 更新日期：2026-08-31
>
...
- `0003_followup_confirmation.sql` 已实现来源、草稿/修订、正式跟进、更正、参与人/商机关联、事实/证据、幂等、审计、领域事件和 Outbox 的物理基线；持久化 Store 与 REST API 已贯通，Web 确认工作台仍在实施；
- 所有 27 张当前业务表使用复合租户外键，并由 PGlite 行为测试与真实 PostgreSQL CI 从空库重建；
...
- `0004`–`0006` 尚未实现；`0003` 已完成 schema、数据库不变量、深 Store 和 REST API，Web 确认工作台、分析动作、通知投递、周报问数和导入不能误称已经落地。
...
联系人换组织与跨组织场景首期保留完整任职历史，但只实现当前/历史查询和有效期约束，不扩展成通用主数据管理。逻辑模型、状态机、租户边界、物理类型、索引和迁移顺序已经闭合；租户/身份、经营核心和跟进确认 schema 已实现，经营对象 repository/API/目录页面已经贯通。后续继续完成 `0003` 的 Store/API/Web，并通过 `0004`–`0006` 补齐分析动作、通知投递、周报问数和导入；尚未确认的周期、阈值和部署形态均被隔离为版本化配置或 adapter，不要求推翻核心表。
