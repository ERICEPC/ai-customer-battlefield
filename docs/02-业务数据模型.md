...
- `0004`–`0006` 尚未实现；`0003` 已完成 schema、数据库不变量、深 Store、REST API 和 Web 确认工作台，分析动作、通知投递、周报问数和导入不能误称已经落地。
...
联系人换组织与跨组织场景首期保留完整任职历史，但只实现当前/历史查询和有效期约束，不扩展成通用主数据管理。逻辑模型、状态机、租户边界、物理类型、索引和迁移顺序已经闭合；租户/身份、经营核心和跟进确认 schema 已实现，经营对象 repository/API/目录页面与跟进确认 Store/API/Web 已经贯通。后续通过 `0004`–`0006` 补齐分析动作、通知投递、周报问数和导入；尚未确认的周期、阈值和部署形态均被隔离为版本化配置或 adapter，不要求推翻核心表。
