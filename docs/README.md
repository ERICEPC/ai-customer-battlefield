...
V1-A 数据与目录基础已经验收；V1-B 已完成跟进确认公共契约、核心端口与 `0003` schema，但持久化 Store、API/Web 确认事务尚未贯通。完整 V1 尚未完成，分析/动作闭环、真实模型、通知、配置管理、周报、管理问数、导入和生产部署按后续连续增量实施，共用当前领域模型与架构 seam。
