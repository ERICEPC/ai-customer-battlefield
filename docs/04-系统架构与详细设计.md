...
当前角色工作台、建议/动作查询和作战地图读模型在 Kysely adapter 内落实当前有效 `entity_assignments` 谓词，并始终保留显式 `tenant_id` 过滤与强制 RLS；管理问数进一步通过共享 active-assignment SQL 片段生成主体、授权和对象范围，避免三个读路径发生时间语义漂移。负责人/协作人可读取责任对象的建议并执行接受/拒绝，只读取并流转由本人负责的正式动作；`management_observer` 的动作列表仍只展示开放动作，可按精确 ID 只读打开当前观察对象的计划中、进行中或已完成动作，以保证问数证据可核验，已取消动作统一按不可见处理，`canTransition` 始终为 false。管理观察者不能替代销售做建议决策或动作流转。建议响应中的 `canDecide` 和动作响应中的 `canTransition` 把同一服务端授权结果显式传给前端；可选责任人目录还要求调用者对指定 `entityId` 具备当前负责人/协作人关系，并只返回该对象当前有效的负责人/协作人。按 ID 精确读取、分页列表与写事务加锁都在服务端重新鉴权，前端深链只携带标识，不能扩大授权。
...
- 创建/确认/状态变化及管理问数执行要求 `Idempotency-Key`；
...
| `POST /management-queries` | 携带 `Idempotency-Key` 执行 strict `sales_weekly_progress` 受控查询 |
...
当前实现只有 `sales_weekly_progress` 一个显式 capability。框架无关应用用例负责校验最长 31 天区间、一次性读取服务端时钟并取 `data_cutoff_at = min(period_end, query_now)`；`period_end` 是严格不含的上界。Kysely adapter 在 transaction-local RLS 内执行查询。销售本人范围来自查询时刻仍有效的 `owner/collaborator` 关系；管理者范围是目标销售当前责任关系与 actor 当前 `management_observer` 关系的对象交集。主体不存在、已离任、未来生效、无责任或越权均使用同一 not-found 语义，拒绝客户端覆盖 tenant、actor、scope 或 SQL。

聚合直接读取正式跟进、有效事实、阶段历史、动作状态历史和作战状态版本。开放/逾期动作按 `data_cutoff_at` 之前最后一条状态历史重建，计划时间等于截止点也计为逾期；作战状态从不可变版本表回放截止点最后版本，不用当前指针污染历史问数。非动作事实证据只跳到重新鉴权的对象作战页，动作与作战状态链接严格绑定响应中的 evidence ID 和 entity ID；证据摘要在 repository 内确定性截断到 500 字符。单次执行最多处理 500 个授权对象及每类 5000 条事件，其中已完成与开放动作共用同一个 `action` 总量上限；超限返回 `MANAGEMENT_QUERY_RESULT_LIMIT_EXCEEDED`（HTTP 422）且不写查询审计，避免先截断后伪装成完整答案。

执行端点强制接收合法 `Idempotency-Key`。每次调用先按当前时刻重新验证主体，并重算当前授权对象集合；客户端稳定请求字段、actor、capability、scope kind 和排序后的授权对象 ID 共同形成请求哈希。只有当前范围与首次执行一致时才允许回放，权限撤销直接使用统一 not-found，范围变化则返回冲突，不能凭旧键读取已失权证据。幂等预占、业务读取、最小查询审计和完整响应快照在同一个租户事务内完成。相同键和相同逻辑请求会直接回放原 `queryId`、截止时间和结果，不重新聚合或重复审计；相同键复用于不同请求返回 `MANAGEMENT_QUERY_IDEMPOTENCY_CONFLICT`（HTTP 409），失败关闭或超限事务不会留下半成品预占。审计只保存 capability、主体、周期、范围、计数和指标，不保存证据正文。真实 PostgreSQL 18 CI 还以并发同键调用验证唯一响应/审计，并验证责任交集和历史动作查询使用现有索引。未来 Agent 或聊天入口只能把自然语言映射到这个 strict 端口或其他已发布 capability，HTTP 和数据库实现无需随 Agent 底座变化。
