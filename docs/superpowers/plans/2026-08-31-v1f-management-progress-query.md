...
- Every headline metric is deterministic. `periodEnd` is exclusive, historical actions and battle states are reconstructed at the cutoff, and planned time equal to the cutoff is overdue. Entity highlights contain typed evidence references and identifier-bound application deep links; missing cutoff analysis is reported as a data gap, never interpreted as low risk.
- One run is bounded to 500 authorized entities and 5000 rows per event kind; completed and open actions share the same combined `action` bound. Exceeding either limit returns an explicit 422 result-limit error before audit persistence; evidence labels are deterministically capped at 500 characters.
- Every execution requires an `Idempotency-Key`. Ambiguous network or response failures reuse the original key and atomically replay the original response without a second audit. Reusing the key for a different logical request returns 409; a scope-conflict 409 is explained and requires a user-confirmed fresh-key execution. Replay occurs only after current subject/entity authorization is recomputed, and the current scope fingerprint is part of the request hash, so a revoked or changed scope cannot recover stale evidence.
...
Evidence kinds are limited to `followup`, `fact`, `stage_change`, `action` and `battle_state`. Follow-up/fact/stage evidence targets the authorized entity battle page; action and battle-state links must contain the exact evidence ID and authorized entity ID declared by the response. A link cannot expand authorization. A management observer can open planned, in-progress or historical completed actions read-only without adding completed actions to the open-action list; cancelled actions remain private.
...
5. Implement one tenant transaction with an explicit server timestamp and a shared active-assignment SQL fragment reused by subject, authorization and entity scope queries. Use parameterized Kysely/SQL only.
...
**Local evidence:** The RED database suite first failed because no management-query adapter existed. The GREEN adapter resolves self and observed-portfolio subjects from a shared active-assignment fragment, applies the same scope to every entity row, attributes action metrics to the formal owner, reconstructs open/overdue action and battle-state history at the exact exclusive cutoff, returns bounded typed evidence and explicit missing-analysis gaps, and writes only aggregate audit metadata. Future, ended, unassigned, same-tenant unobserved and cross-tenant activity is absent. Result-limit failures write no query audit. A completed idempotent execution replays its stored response without recomputation or a second audit only after current authorization is recomputed; scope expansion conflicts and access revocation returns not-found. The combined action bound and cancelled observer detail are covered by regression tests. The complete database package passes 105/105 with typecheck, Biome, public-boundary and diff checks green. PostgreSQL 18 concurrent replay/plan/compatibility evidence remains part of Task 5 CI acceptance.
...
**Local evidence:** The new E2E suite first received 404 because neither endpoint existed. The GREEN module now exposes only strict `GET /api/v1/management-query-subjects` and `POST /api/v1/management-queries`, derives the actor from the existing server-side development identity boundary, rejects tenant/user/scope/SQL overrides, requires a valid idempotency header, maps invalid input to 400, unified missing/unauthorized subjects to 404, key conflicts to 409, bounded-processing overflow to 422, and absent persistence to 503. E2E proves same-key replay returns the original result and leaves exactly one audit. Long database evidence is summarized before strict response validation. The complete API E2E set passes 36/36 with API typecheck and Biome green.
...
1. Write RED client/component tests for authorized subject loading, “本周” interval display, query submission, metrics, evidence links, data gaps, empty scope, validation, slow loading, retry and in-flight duplicate suppression.
...
**Local evidence:** Client and component suites first failed because `/ask` modules and navigation did not exist. The GREEN page loads every authorized subject page, defaults to an explicit UTC Monday-to-next-Monday interval, validates ranges before submission, renders six deterministic metrics, scoped entity highlights, typed evidence links, result cutoff/query ID and truthful analysis gaps, and handles empty scope, slow loading, retries and an in-flight lock that prevents duplicate queries/audits. Both error-state retry controls reuse the same failed-attempt key for ambiguous failures; successful explicit regeneration uses a new key. A 409 scope conflict is preserved as a typed API error, explained to the user, and requires an explicit fresh-key query against the current scope rather than looping or silently retrying. The desktop and 390×844 synthetic-manager browser flow completed query → evidence → re-authorized read-only action. Final responsive measurements were `innerWidth=390`, `documentElement.scrollWidth=375`, `body.scrollWidth=375`; composer controls were 46px, mobile navigation 55px and evidence links 44px, with no browser console errors on the final localhost flow. Web passes 90/90, typecheck, Biome and production build including static `/ask`.
